...
print(memoria_cache_conj)
print(bloco_backup_final)
while True:
    ...
    for i in range(0, int(n_linha_cache/n_conj_cache)):  # verifica se o bloco se encontra na cache
        if memoria_cache_conj[subconjunto][i][0] == tag: # se o bloco estiver na cache, o programa apenas fará a leitura
    ...
    if boole == False:  # se o bloco não estiver na cache o programa irá no endereço especificado na ram e atualizará a linha
        ...
        if bloco_backup_final[subconjunto][linha] != '3':  # se tiver sido atualizado, ele irá atualizar a memória ram com os
